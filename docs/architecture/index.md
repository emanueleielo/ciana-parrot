...
| **Agent** | `src/agent.py` | Creates the LangGraph agent via `create_deep_agent()` with LLM, memory files, skills, custom tools, and MCP tools. Uses `WorkspaceShellBackend` for sandboxed file and shell operations. |
...
| **Scheduler** | `src/scheduler.py` | Background asyncio task that polls `scheduled_tasks.json`. Supports three task types: `cron` (croniter expressions), `interval` (seconds), `once` (ISO timestamp). Executes tasks by invoking the agent, then sends results to the originating channel/chat. |
